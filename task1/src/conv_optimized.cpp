                ...
                for (int ky = 0; ky < K; ++ky) {
                    ...
                    for (int kx = 0; kx < K; ++kx) {
                        ...
                        for (; ox + 32 <= j_end; ox += 32) {
                            ...
                            if (ky == 0 && kx == 0) {
                                _mm256_storeu_ps( out_row + ox, _mm256_mul_ps(v0, kernel));
                                _mm256_storeu_ps( out_row + ox + 8, _mm256_mul_ps(v1, kernel));
                                _mm256_storeu_ps( out_row + ox + 16, _mm256_mul_ps(v2, kernel));
                                _mm256_storeu_ps( out_row + ox + 24, _mm256_mul_ps(v3, kernel));
                            } else {
                                __m256 o0 = _mm256_loadu_ps(out_row + ox);
                                __m256 o1 = _mm256_loadu_ps(out_row + ox + 8);
                                __m256 o2 = _mm256_loadu_ps(out_row + ox + 16);
                                __m256 o3 = _mm256_loadu_ps(out_row + ox + 24);
                        ...
                        for (; ox + 8 <= j_end; ox += 8) {
                            ...
                            if (ky == 0 && kx == 0) {
                                _mm256_storeu_ps( out_row + ox, _mm256_mul_ps(input, kernel));
                            } else {
                                __m256 output = _mm256_loadu_ps(out_row + ox);
                                output = _mm256_fmadd_ps(input, kernel, output);
                                _mm256_storeu_ps(out_row + ox, output);
                            }
                        }

                        for (; ox < j_end; ++ox) {
                            float value = in_row[ox + kx] * w;
                            if (ky == 0 && kx == 0) out_row[ox] = value;
                            else out_row[ox] += value;
                        }
                    }
                }
            }
