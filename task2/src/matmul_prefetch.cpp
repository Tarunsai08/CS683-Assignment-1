...
// Examples:
//   0  -> no B prefetch
//   4  -> prefetch 4 B rows ahead
//   8  -> prefetch 8 B rows ahead
//   16 -> prefetch 16 B rows ahead
//   32 -> prefetch 32 B rows ahead
//   64 -> prefetch 64 B rows ahead
//
// This is the variable you should change when plotting
...
static inline void compute_tile_sizes(int K, int& BM, int& BN) {
    ...
    BM = static_cast<int>(
        max_rows - static_cast<std::size_t>(BN)
    );
...
static inline void block4_simd_prefetch(
    const float* a_row,
    const float* b0,
    const float* b1,
    const float* b2,
    const float* b3,
    const float* b_prefetch,
    int K,
    float* out
) {
    ...
    for (int j0 = 0; j0 < N; j0 += BN) {
        ...
        for (int i0 = 0; i0 < M; i0 += BM) {
            ...
            for (int i = i0; i < iEnd; ++i) {
                ...
                float* c_row =
                    C + static_cast<std::size_t>(i) * ldc;
